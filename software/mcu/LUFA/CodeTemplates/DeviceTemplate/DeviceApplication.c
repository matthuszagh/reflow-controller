...
void SetupHardware(void)
{
  ...
  /* Start the PLL to multiply the 2MHz RC oscillator to 32MHz and switch the CPU core to run from
   * it */
  XMEGACLK_StartPLL(CLOCK_SRC_INT_RC2MHZ, 2000000, F_CPU);
  XMEGACLK_SetCPUClockSource(CLOCK_SRC_PLL);

  /* Start the 32MHz internal RC oscillator and start the DFLL to increase it to 48MHz using the USB
   * SOF as a reference */
